function form_data_init() {
    var page = parseInt(QUERY_STRING_PARAMS.get("page"));
    if(!page) {
        page = 1;
    }
    var txtFilter = QUERY_STRING_PARAMS.get("txtFilter");
    if(txtFilter) {
        $("#txtFilter").val(txtFilter);
    } else {
        txtFilter = "";
    }
    var grpFilter = QUERY_STRING_PARAMS.get("grpFilter");
    if(grpFilter){
        $("grpFilter").val(grpFilter);
    } else {
        grpFilter = "Semua";
    }
    return {
      page: page,
      txtFilter: txtFilter,
      grpFilter: grpFilter}
}


$(document).ready(function(){
    $("#btnFilter").click(function() {
        var queryString = "?";
        var page = parseInt(QUERY_STRING_PARAMS.get('page'));
        if(page) {
            queryString += "page="+page;
        } else {
            queryString += "page=1";
        }
        var textfilter = $('#txtFilter').val();
        if(textfilter.trim() !== "") {
            queryString += "&txtFilter="+textfilter;
        }
        var groupfilter = $('#grpFilter').val();
        if(groupfilter !== "Semua") {
            queryString += "&grpFilter="+groupfilter;
        }
        if(queryString === "?") {
            window.location.href = new URL(MAIN_URL);
        } else {
            window.location.href = new URL(`${MAIN_URL}${queryString}`);
        }
    });
});
